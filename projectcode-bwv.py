...
def encode(text_path,video_path,max_second):
    ...
    for c in str1:
        ...
        even_sum=0
        for num in b:
            if(ord(num)-ord('0')==0):even_sum+=1
            elif(ord(num)-ord('0')==1):odd_sum+=1
        if(odd_sum%2==0):b+='1'
        else:b+='0'
        #print(b)
        for b1 in b:
...
def decode(video_path,pic_path,txt_path,check_path):
    ...
    while countx < size:
        ...
        if np.sum(img[countx:countx + cube, county:county + cube]) <60000:  # 这里相当于是取小像素块的平均值，考虑到后面手机拍摄可能会产生色差
            bin1 = bin1 + '1'
        else:
            bin1 = bin1 + '0'
        #print(bin1)
            ...
            if (len(contents2) > i):  # 如果原本的文件有第i行
                ...
                for c in line:  # 每一行中的每一个字符
                    ...
                    if (len(contents2[i]) < j + 1):  # 为防止越界 有可能原本的文件比较短
                        ...
                        for k in s1:
                            if(ord(k)-ord('0')==0):even+=1
                            else:odd+=1

                        if even%2==0 and vaild[index]=='0':judge='00000000'
                        if even%2==1 and vaild[index]=='1':judge='00000000'
